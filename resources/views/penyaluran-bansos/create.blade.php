            ...
            <div class="col-md-10">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('penyaluran-bansos.store') }}" enctype="multipart/form-data">
                            @csrf

                            <div class="form-group{{ $errors->has('penerima_bansos_id') ? ' has-danger' : '' }}">
                                <label class="form-control-label">Pilih Penerima <span class="text-danger">*</span></label>
                                <select name="penerima_bansos_id" id="penerima-select" class="form-control{{ $errors->has('penerima_bansos_id') ? ' is-invalid' : '' }}" required onchange="updateProgram(this.value)">
                                    ...
                                    @foreach($penerimaBansos as $penerima)
                                        <option value="{{ $penerima->id }}" 
                                            ...
                                            {{ $penerima->nama_lengkap }} - {{ $penerima->nik }} ({{ $penerima->programBansos->nama_program }})
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group{{ $errors->has('program_bansos_id') ? ' has-danger' : '' }}">
                                ...
                                <select name="program_bansos_id" id="program-select" class="form-control{{ $errors->has('program_bansos_id') ? ' is-invalid' : '' }}" required>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group{{ $errors->has('nominal_diterima') ? ' has-danger' : '' }}">
                                        ...
                                        <input type="number" name="nominal_diterima" class="form-control{{ $errors->has('nominal_diterima') ? ' is-invalid' : '' }}" value="{{ old('nominal_diterima') }}" min="0" step="0.01" required>
                                        @include('alerts.feedback', ['field' => 'nominal_diterima'])
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group{{ $errors->has('metode_penyaluran') ? ' has-danger' : '' }}">
                                        <label class="form-control-label">Metode Penyaluran <span class="text-danger">*</span></label>
                                        <select name="metode_penyaluran" id="metode-select" class="form-control{{ $errors->has('metode_penyaluran') ? ' is-invalid' : '' }}" required onchange="toggleMetodeFields(this.value)">
                                            <option value="">-- Pilih Metode --</option>
                                            <option value="transfer" {{ old('metode_penyaluran') == 'transfer' ? 'selected' : '' }}>Transfer</option>
                                            <option value="tunai" {{ old('metode_penyaluran') == 'tunai' ? 'selected' : '' }}>Tunai</option>
                                            <option value="voucher" {{ old('metode_penyaluran') == 'voucher' ? 'selected' : '' }}>Voucher</option>
                                            <option value="barang" {{ old('metode_penyaluran') == 'barang' ? 'selected' : '' }}>Barang</option>
                                        </select>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group{{ $errors->has('no_rekening') ? ' has-danger' : '' }}">
                                            <label class="form-control-label">No Rekening <span class="text-danger">*</span></label>
                                            <input type="text" name="no_rekening" class="form-control{{ $errors->has('no_rekening') ? ' is-invalid' : '' }}" value="{{ old('no_rekening') }}">
                                            @include('alerts.feedback', ['field' => 'no_rekening'])
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group{{ $errors->has('nama_bank') ? ' has-danger' : '' }}">
                                            <label class="form-control-label">Nama Bank <span class="text-danger">*</span></label>
                                            <input type="text" name="nama_bank" class="form-control{{ $errors->has('nama_bank') ? ' is-invalid' : '' }}" value="{{ old('nama_bank') }}">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group{{ $errors->has('tanggal_penyaluran') ? ' has-danger' : '' }}">
                                        <label class="form-control-label">Tanggal Penyaluran <span class="text-danger">*</span></label>
                                        <input type="date" name="tanggal_penyaluran" class="form-control{{ $errors->has('tanggal_penyaluran') ? ' is-invalid' : '' }}" value="{{ old('tanggal_penyaluran', date('Y-m-d')) }}" required>
                            ...
                            <div class="form-group{{ $errors->has('bukti_penyaluran') ? ' has-danger' : '' }}">
                                ...
                                <input type="file" name="bukti_penyaluran" class="form-control-file{{ $errors->has('bukti_penyaluran') ? ' is-invalid' : '' }}" accept=".pdf,.jpg,.jpeg,.png">
                            ...
                            <div class="form-group{{ $errors->has('catatan') ? ' has-danger' : '' }}">
                                ...
                                <textarea name="catatan" class="form-control{{ $errors->has('catatan') ? ' is-invalid' : '' }}" rows="3" placeholder="Catatan tentang penyaluran (opsional)">{{ old('catatan') }}</textarea>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const penerimaSelect = document.getElementById('penerima-select');
        if (penerimaSelect.value) {
            updateProgram(penerimaSelect.value);
        }
